...
namespace Catalog.Web.Areas.Shop.Controllers
{
    public class CartController : BaseController
    {
        ...
        public ActionResult Cart()
        {
            ...
            var cart = ParticipantContext.Carts.Single(where: "Cookie = @0", parms: cookie.Value);
            if (cart == null)
            {
            ...
            var items = ParticipantContext.CartItems.All(where: "CartId = @0", parms: cart.Id);

            if (items.Count() > 0)
            {
                model.Items = Mapper.Map<IEnumerable<CartItem>, IEnumerable<CartItemModel>>(items);

                var products = ParticipantContext.Products.All(where: "Id IN (" + items.CommaSeparate(i => i.ProductId) + ")").ToDictionary(p => p.Id);
                foreach (var item in model.Items)
                {
                    ...
                    item.ProductParticipantName = ParticipantCache.Participants[products[item.ProductId].ParticipantId].FullName;
                }
        ...
        public ActionResult Cart(int id, int quantity)
        {
            ...
            new Thread(() =>
            {
                var cartItem = ParticipantContext.CartItems.Single(id);
                cartItem.Quantity = quantity;
                ParticipantContext.CartItems.Update(cartItem);
        ...
        public ActionResult Cart(int id)
        {
            ...
            {
                var cartItem = ParticipantContext.CartItems.Single(id);
                ParticipantContext.CartItems.Delete(cartItem);
        ...
        public ActionResult Checkout(CheckoutModel model)
        {
            ...
            order.OrderNumber = ParticipantContext.OrderNumbers.Next();
        ...
        void AddToNewCart(HttpCookie cookie, int count, int? id)
        {
            ...
            using (var uow = new IaSUnitOfWork())
            {
                // ** Prototype Pattern
        ...
        void AddToExistingCart(HttpCookie cookie, int count, int? id)
        {
            ...
            var item = ParticipantContext.CartItems.Single(where: "CartId = @0 AND ProductId = @1", parms: new object[] { cart.Id, id });
            if (item != null)
            {
                item.Quantity += count;
                ParticipantContext.CartItems.Update(item);
            }
            else  // cart item has default values set
            {
                ...
                ParticipantContext.CartItems.Insert(item);
            }
        }
